...
function setupNotifications() {
    chat.server.getNotificationList().done(function (result) {
        //spracovanie JSON dat a vygenerovanie jednotlivych notifikacii na webstranke
        ...
        for (var i = 0; i < notifications.length; i++) {
            ...
            $('.notifications-bar').append(clone);
            clone.show();
        }
        //po kliknuti na notifikaciu pred presmerovanim na inu stranku najprv upozorni server
        //na "precitanie" notifikacie- not.sa uz nebude zobrazovat ako "neprecitana"
...
//max pocet popupov, ktore sa mozu zobrazit
var total_popups = 0;
...
$.connection.hub.start().done(function () {
    //po vytvoreni spojenia zavola funkciu,
    //ktora si vypyta friendlist a vytvori prislusne ikony v sidebare
    setupChat();
    //vypyta si od servera nove notifikacie, vlozi ich do notifikacneho panela
    //a pokial panel obsahuje nevidene notifikacie, upozorni uzivatela
    setupNotifications();
});
